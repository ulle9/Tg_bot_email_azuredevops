...
def read_email(tg_id, check_dict):
...
def start_check(message):
    if message.from_user.id in allowed_users:
        if not process_dict[message.from_user.id][0]:
            process_dict[message.from_user.id] = [Process(target=read_email, args=(message.from_user.id, check_dict)),
                                                  str(datetime.now())]
            process_dict[message.from_user.id][0].start()
        else:
...
def stop_check(message):
    if message.from_user.id in allowed_users and process_dict[message.from_user.id][0]:
        process_dict[message.from_user.id][0].terminate()
...
if __name__ == '__main__':
    ...
    for i in allowed_users:
        ...
        check_dict[i] = manager.list([manager.list([]),
                                      manager.list([]),
                                      manager.dict({'reg_flag': True, 'send_flag': True,
                                                    'reg_item_num': False, 'sent_to': manager.list([])}),
                                      manager.dict({'process_started': False, 'errors': 0, 'read_emails': 0,
